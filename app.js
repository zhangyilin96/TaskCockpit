...
function activeToast(){const dialog=topOpenDialog();if(!dialog)return byId("toast");let toast=dialog.querySelector(".dialog-toast");if(!toast){toast=document.createElement("div");toast.className="toast dialog-toast";toast.setAttribute("role","status");toast.setAttribute("aria-live","polite");dialog.appendChild(toast)}return toast}
function showToast(message){const toast=activeToast();document.querySelectorAll(".toast.show").forEach(item=>{if(item!==toast)item.classList.remove("show")});toast.textContent=message;toast.classList.add("show");clearTimeout(toastTimer);toastTimer=setTimeout(()=>toast.classList.remove("show"),2000)}
